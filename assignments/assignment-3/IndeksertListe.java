public class IndeksertListe<T> extends LenkeListe<T> {
    ...
    public void leggTil(int pos, T element) throws UgyldigListeindeks{
        ...
        if(pos == 0 && head == null){
            //Sette inn aller første element i 
            head = newNode;
        }else if(pos == 0 && tail == null){
            //Sette inn element i posisjonen til første node
            newNode.next = head;
            head.prev = newNode;
            tail = head;
            head = newNode;
        }else if(pos == 1 && tail == null){
            //Sette inn element i posisjon til siste element
            head.next = newNode;
            newNode.prev = head;
            tail = newNode;
        }else if(pos == 0){
            ...
            //Node før node[pos] må peke på det nye noden
